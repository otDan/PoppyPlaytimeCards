...
namespace PoppyPlaytimeCards.Card
{
    internal class HuggyWuggyCard : CustomCard
    {
        ...
        public override void OnAddCard(Player player, Gun gun, GunAmmo gunAmmo, CharacterData data, HealthHandler health, Gravity gravity, Block block, CharacterStatModifiers characterStats)
        {
            ...
            tallPlayerEffect.ratio *= 0.25f;
            tallPlayerEffect.yOffset += 0.25f;
            tallPlayerEffect.ResetScale();
            tallPlayerEffect.MakeTall();
            player.gameObject.transform.GetChild(3).gameObject.GetComponentInChildren<LegRaycasters>().force *= 1.65f;
        }
        ...
        internal class TallPlayerEffect : MonoBehaviour
		{
			...
			private void Update()
            {
                ...
                ResetTimer();
                if (Math.Abs(gameObject.transform.localScale.x - gameObject.transform.localScale.y) > 0.001) return;
                orig_scale = gameObject.transform.localScale;
                MakeTall();
            }
